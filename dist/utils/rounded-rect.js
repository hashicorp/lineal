...
const arc = (rx, ry, hDir, vDir) => `a ${rx},${ry} 0 0 1 ${rx * hDir},${ry * vDir}`;
 ...
 */
function roundedRect(rect, radii, safe = false) {
  ...
  return [move(rect.x, rect.y + rtly), arc(rtlx, rtly, 1, -1), hLine(rect.width - rtlx - rtrx), arc(rtrx, rtry, 1, 1), vLine(rect.height - rtry - rbry), arc(rbrx, rbry, -1, 1), hLine(-rect.width + rbrx + rblx), arc(rblx, rbly, -1, -1), vLine(-rect.height + rbly + rtly), 'Z'].join(' ');
}
