...
export default class extends Component {
  ...
  @cached get bespokeStack() {
    ...
    const analyzed = grouped.map(series => ({
      name: series[0],
      sources: series[1],
      total: sum(series[1].mapBy('value')),
      totalFossilFuels: sum(series[1].slice(0, 3).mapBy('value')),
    }));

    // And sort by total descending
    return analyzed.sortBy('total').reverse();
  }
  ...
  @cached get sources() {
    return this.bespokeStack[0].sources.mapBy('source');
  }
