...
```gts
import Fluid from "@lineal-viz/lineal/components/lineal/fluid";
    ...
    <Fluid as |width height|>
      <svg width={{width}} height={{height}}>
        {{#let
          ...
          (scaleLinear domain="0..100" range=(array height 0))
          as |xScale yScale|
        }}
          {{! Chart content using xScale and yScale }}
        {{/let}}
      </svg>
    </Fluid>
  </div>
...
Scale helpers like `scale-linear` create a **new scale instance on every render**. Mark components call an internal `qualifyScale()` function that schedules domain updates. When Fluid updates dimensions → scales recreate → marks update domains → triggers re-render → Fluid updates → infinite loop.
...
import Fluid from "@lineal-viz/lineal/components/lineal/fluid";
import Line from "@lineal-viz/lineal/components/lineal/line";
import { array } from "@ember/helper";
...
import { extent } from "d3-array";
import Fluid from "@lineal-viz/lineal/components/lineal/fluid";
import Axis from "@lineal-viz/lineal/components/lineal/axis";
import GridLines from "@lineal-viz/lineal/components/lineal/grid-lines";
import Line from "@lineal-viz/lineal/components/lineal/line";
