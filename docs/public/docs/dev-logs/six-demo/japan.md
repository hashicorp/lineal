        ...
        {{#let
          ...
          {{#if this.activeStackSlice}}
            <div
              ...
              {{style
                --x=(str
                  (xScale.compute this.activeStackSlice.datum.datum.Year)
                )
              }}
            >
              ...
              <dl>
                {{#each stacked.stackedCategories as |cat|}}
                  {{#let
                    (find-by "datum.source" cat this.activeStackSlice.data)
                    as |d|
                  }}
...
export default class extends Component {
  ...
  load = task(async () => {
    const req = await fetch(
      "/datasets/japan-energy-mix-over-time-records.json",
    );
